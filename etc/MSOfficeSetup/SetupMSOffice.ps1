...
$file = Get-Item -Path "$PSScriptRoot\officedeploymenttool*.exe"
if ($file) {
    $installerFile  = $file.FullName
    $installerFile | Write-Host -ForegroundColor Green
} else { 
...
Remove-Item -Path $installerFile -ErrorAction SilentlyContinue # Test-Path -Path $installerFile
